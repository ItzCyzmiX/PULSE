...
const patVariableFunction = /([{; =])([a-zA-Z$][a-zA-Z0-9$]{0,2})\.([a-zA-Z$][a-zA-Z0-9$]{0,2})\(/g;
...
async function exract(videoID) {
	...
	if (mat) {
		...
		parseFormats(adaptiveFormats, file, encSignatures, patSigEncUrl, patSignature);
	} else {
	...
	if (encSignatures.size > 0) {
		...
		if (LOGGING) console.log(`Decipher signatures: ${encSignatures.size}, video: ${file}`);
...
function parseFormats(formats, file, encSignatures, patSigEncUrl, patSignature) {
	if (formats) {
	...
	if (!decipherFunctionName || !decipherFunctions) {
		...
		try {
			...
			if (mat) {
				...
				if (LOGGING) console.log(`Main Decipher Function: ${mainDecipherFunct}`);
				...
				while ((mat = patVariableFunction.exec(mainDecipherFunct))) {
					...
					let startIndex = javascriptFile.indexOf(variableDef) + variableDef.length;
					let bracesVariable = 1;
					let i1 = startIndex;
					while (i1 < javascriptFile.length) {
						if (bracesVariable === 0) {
							decipherFunctions += `${variableDef}${javascriptFile.substring(startIndex, i1)};`;
							break;
						}
				...
				while ((mat = patFunction.exec(mainDecipherFunct))) {
					...
					startIndex = javascriptFile.indexOf(functionDef, mat.index) + functionDef.length;
					let bracesFunction = 0;
					let i2 = startIndex;
					while (i2 < javascriptFile.length) {
						if (bracesFunction === 0 && startIndex + 5 < i2) {
							decipherFunctions += `${functionDef}${javascriptFile.substring(startIndex, i2)};`;
							break;
						}
...
export default async function getStreamUrl(videoID) {
	let ok = false;
	let retry = 3;
	let i = 0;
	try {
